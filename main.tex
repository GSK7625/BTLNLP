...
\begin{titlepage}
    ...
    \begin{flushleft}
        ...
        \begin{minipage}{0.8\textwidth}
            ...
            \textbf{Giảng viên hướng dẫn:} ThS. Nguyễn Đình Quý
        \end{minipage}
    \end{flushleft}
...
\begin{table}[H]
  ...
  \begin{tabular}{lllc}
    ...
    \midrule
    \textbf{Đỗ Trung Kiên} & Khảo sát, làm sạch dữ liệu, mã hóa & File \texttt{preprocess.py}, dataset sạch & 20\% \\
    \textbf{Lê Anh Đức} & Xây dựng baseline BM25 cho Retriever & File \texttt{baseline\_bm25.py} & 20\% \\
    \textbf{Dương Tiến Hùng} & Thiết kế pipeline huấn luyện XLM-R & File \texttt{train.py}, weights checkpoint & 20\% \\
    \textbf{Đinh Xuân Nam} & Thiết kế module phân tích lỗi tự động & File \texttt{error\_analysis.py, .csv} & 20\% \\
    \bottomrule
  \end{tabular}
  \label{tab:phancong}
\end{table}
...
Các mô hình và pipeline được đánh giá đồng bộ trên tập kiểm thử sạch gồm \textbf{500 mẫu} ngẫu nhiên của tệp \texttt{test\_clean.json}. Mọi thử nghiệm đều cấu hình cố định \texttt{max\_seq\_length = 256} trên môi trường CPU tiêu chuẩn.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{results/figures_500/fig1_em_f1_comparison.png}
  \caption{So sánh Exact Match và Token F1 giữa các mô hình (500 mẫu test)}
  \label{fig:emf1}
\end{figure}
...
\begin{table}[H]
  \centering
  \small
  \caption{Kết quả đo lường hiệu năng và tốc độ xử lý của các mô hình (500 mẫu test)}
  \begin{tabular}{lcccl}
    \toprule
    \textbf{Mô hình} & \textbf{EM} & \textbf{Token F1} & \textbf{Độ trễ (Inference/mẫu)} & \textbf{Vai trò hệ thống} \\
    \midrule
    \textbf{B2: XLM-R Pretrained} & 44.60\% & 70.39\% & $\sim$110 ms & Baseline zero-shot cross-lingual. \\
    \textbf{M1: XLM-R Fine-tuned} & \textbf{60.60\%} & \textbf{81.05\%} & $\sim$110 ms & Phương pháp chính (độc lập). \\
    \textbf{Pipeline: BM25 + B2} & 37.80\% & 61.40\% & $\sim$115 ms & BM25 Top-5 + Pretrained Reader. \\
    \textbf{Pipeline: BM25 + M1} & 53.80\% & 71.95\% & $\sim$115 ms & BM25 Top-5 + M1 + Rank Penalty. \\
    \bottomrule
  \end{tabular}
  \label{tab:ketqua}
\end{table}

\textbf{Nhận xét:} Fine-tune M1 tăng EM từ 44.6\% lên 60.6\% (+16\%) và F1 từ 70.39\% lên 81.05\% (+10.66\%), chứng minh hiệu quả của việc thích nghi mô hình với tiếng Việt. Pipeline (BM25+M1) đạt EM=53.8\% — thấp hơn M1 Oracle (60.6\%) do lỗi Retriever (Recall@5 = 93.4\%, còn $\sim$6.6\% câu hỏi BM25 không tìm được đúng context) nhưng vẫn vượt Pipeline dùng Pretrained Reader (EM 37.8\%).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{results/figures_500/fig7_retriever_accuracy.png}
  \caption{Đường cong BM25 Recall@K theo ngưỡng K (500 mẫu test)}
  \label{fig:recallk}
\end{figure}
...
\begin{table}[H]
  \centering
  \small
  \caption{Bảng Hit@K của BM25 Retriever}
  \begin{tabular}{ccp{7cm}}
    \toprule
    \textbf{Ngưỡng K} & \textbf{Hit@K (Recall@K)} & \textbf{Nhận xét} \\
    \midrule
    K = 1 & 74.3\% & Chỉ dùng kết quả tốt nhất --- độ bao phủ thấp \\
    K = 2 & 86.6\% & Cải thiện đáng kể khi xét thêm 1 ứng cử viên \\
    \textbf{K = 3} & \textbf{93.4\%} & \textbf{Điểm nhóm đang dùng --- cân bằng tốt nhất} \\
    K = 5 & 96.8\% & Tăng nhẹ nhưng gây rủi ro truncation cho Reader \\
    \bottomrule
  \end{tabular}
  \label{tab:recallk}
\end{table}

\textbf{Lý do chọn K=3:} Tăng K lên 5 chỉ cải thiện thêm 3.4\% nhưng làm tổng độ dài input tăng gấp 1.67 lần, tái diễn hiện tượng cắt cụt token (truncation). K=3 là điểm cân bằng tối ưu giữa coverage và chi phí tính toán.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|p{3.2cm}|p{2cm}|c|c|p{3cm}|}
    \hline
    \textbf{STT} & \textbf{Câu hỏi} & \textbf{Đáp án đúng} & \textbf{Top-1 đúng?} & \textbf{Top-3 đúng?} & \textbf{Nhận xét} \\ \hline
...
\textbf{Nhận xét:} BM25 hoạt động tốt với câu hỏi có từ khóa rõ ràng (địa danh, tên phát minh), nhưng thất bại khi câu hỏi liên quan đến tên riêng hiếm gặp hoặc cần suy luận quan hệ --- đây chính là nguyên nhân của $\sim$6.6\% lỗi Retriever còn lại.
...
Khi so sánh \textbf{B2 (Pretrained)} và \textbf{M1 (Fine-tuned)}: EM tăng từ 44.60\% lên 60.60\% (+16\%), F1 tăng từ 70.39\% lên 81.05\% (+10.66\%). Sự cải thiện lớn hơn ở EM cho thấy fine-tune giúp mô hình cắt biên span chính xác hơn --- học cách loại bỏ danh xưng thừa (``Thiếu tướng'', ``Ông'') trước tên riêng --- thay vì chỉ định vị vùng thông tin (đã giỏi nhờ pretrain đa ngôn ngữ).

\subsubsection{Pipeline thấp hơn Reader đơn lẻ --- Hiện tượng Rank Penalty}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{results/figures_500/fig6_pipeline_gain.png}
  \caption{So sánh hiệu năng Reader đơn lẻ (Oracle) và Pipeline BM25+Reader}
  \label{fig:pipelinegain}
\end{figure}

Biểu đồ \ref{fig:pipelinegain} chỉ ra Reader đơn lẻ (M1 Oracle) đạt EM=60.6\% / F1=81.05\%, trong khi Pipeline BM25+M1 chỉ đạt EM=53.8\% / F1=71.95\% --- Pipeline thấp hơn Oracle khoảng 6--9 điểm. Đây là kết quả hoàn toàn hợp lý: trong Pipeline, BM25 phải tự tìm trong toàn bộ kho dữ liệu và đưa Top-5 đoạn vào Reader. Dù BM25 đạt Recall@5 $\approx$ 93.4\%, vẫn còn $\sim$6.6\% câu hỏi mà đoạn Gold không lọt vào Top-5 --- những trường hợp này chắc chắn Pipeline trả lời sai. Để cải thiện cần thay BM25 bằng Dense Retrieval (DPR).

\subsubsection{So sánh baseline và phương pháp chính}
Phương pháp chính (M1) vượt trội rõ ràng so với cả hai baseline: so với B2 (Pretrained) tăng +16\% EM và +10.66\% F1; so với Pipeline+B2 Reader: M1-Oracle có EM cao hơn 22.8\% và F1 cao hơn 19.65\%. Nhóm ưu tiên chọn \textbf{M1 đơn lẻ} làm phương pháp chính (đạt điểm cao nhất), và dùng \textbf{Pipeline+M1} cho kịch bản thực tế (không có Gold Context sẵn).

\subsubsection{Lựa chọn ngưỡng Top-K lý tưởng cho Retriever}
Dựa trên Bảng \ref{tab:recallk}, $K=3$ là điểm cân bằng kỹ thuật lý tưởng nhất giữa độ bao phủ thông tin (Hit@3 = 93.4\%) và rủi ro truncation ngữ cảnh. Tăng K lên 5 chỉ cải thiện thêm 3.4\% nhưng làm tổng input token tăng gấp 1.67 lần.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{results/figures_500/fig4_error_distribution.png}
  \caption{Phân phối loại lỗi của mô hình M1: XLM-R Fine-tuned (trên tập kiểm thử)}
  \label{fig:errordist}
\end{figure}

Thông qua hệ thống phân lớp lỗi tự động từ module \texttt{error\_analysis.py}, nhóm ghi nhận mô hình học sâu M1 tập trung chủ yếu vào các nhóm lỗi liên quan đến biên độ thực thể:
\begin{enumerate}
    \item \textbf{Lỗi biên (Span dư/thiếu - Over/Under-extraction)}: Chiếm \textbf{46.6\%} tổng lượng lỗi (Model định vị đúng vùng thông tin nhưng trích xuất dư/thiếu ký tự hoặc danh xưng).
    \item \textbf{Sai span hoàn toàn (Wrong span)}: Chiếm \textbf{41.7\%} (Model chọn sai phân đoạn trong ngữ cảnh phức tạp chứa nhiều thực thể tương đồng).
    \item \textbf{Nhãn nhiễu (Label noise)}: Chiếm \textbf{11.7\%} (Lỗi khách quan do dữ liệu gốc bị paraphrase hoặc sai lệch dịch máy).
\end{enumerate}

\subsection{Phân rã rủi ro lan truyền của hệ thống Pipeline (Cascading Errors)}
Trong số 500 câu hỏi thử nghiệm, hệ thống Pipeline M1 ghi nhận \textbf{231 mẫu dự đoán sai} (ứng với EM=53.8\%). Dựa trên chỉ số Hit@5 = 93.4\% của Retriever (tương đương 33 mẫu BM25 tìm sai đoạn văn), nhóm tiến hành phân rã rủi ro lan truyền (Root Cause Analysis):
\begin{itemize}
    \item \textbf{Lỗi do Retriever (Retriever Fault)}: Chiếm \textbf{$\sim$14.3\%} (33/231 mẫu) --- Bộ lọc BM25 thất bại hoàn toàn trong việc nạp ngữ cảnh chứa đáp án vào Top-5, khiến Reader không có thông tin đầu vào chính xác để trích xuất.
    \item \textbf{Lỗi do Reader (Reader Fault)}: Chiếm \textbf{$\sim$85.7\%} (198/231 mẫu) --- BM25 nạp đúng ngữ cảnh nhưng mô hình Reader trích xuất sai biên thực thể hoặc nhầm vị trí.
\end{itemize}
\textbf{Lưu ý:} Trong số lỗi Reader, $\sim$11.7\% thực chất là nhãn nhiễu (Label noise --- lỗi do dữ liệu gốc bị paraphrase khi dịch từ SQuAD). Mức trần lý thuyết của hệ thống vì vậy chỉ đạt $\sim$88\%. Việc M1 đạt F1=81.05\% trên Oracle cho thấy hệ thống đang hoạt động sát với hiệu năng tối đa có thể đạt được.
