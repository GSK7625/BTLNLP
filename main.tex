...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{results/figures_5000/fig1_em_f1_comparison.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccl}
    ...
    \midrule
    \textbf{B2: XLM-R Pretrained} & 44.40\% & 66.60\% & $\sim$110 ms & Baseline zero-shot cross-lingual. \\
    \textbf{M1: XLM-R Fine-tuned} & \textbf{56.50\%} & \textbf{76.10\%} & $\sim$110 ms & Phương pháp chính (độc lập). \\
    \textbf{Pipeline: BM25 + B2} & 35.00\% & 53.20\% & $\sim$115 ms & BM25 Top-3 + Pretrained Reader. \\
    \textbf{Pipeline: BM25 + M1} & 36.20\% & 48.40\% & $\sim$115 ms & BM25 Top-3 + M1 + Rank Penalty. \\
    \bottomrule
  \end{tabular}
  }
  \label{tab:ketqua}
...
\textbf{Nhận xét:} Fine-tune M1 tăng EM từ 44.4\% lên 56.5\% (+12.1\%) và F1 từ 66.6\% lên 76.1\% (+9.5\%), chứng minh hiệu quả của việc thích nghi mô hình với tiếng Việt. Pipeline (BM25+M1) đạt EM=36.2\% --- thấp hơn M1 Oracle (56.5\%) vì lỗi Retriever lan truyền khi tập test lớn hơn (Hit@3 (Recall@3) = 93.4\%, $\sim$6.6\% câu hỏi BM25 không tìm được đúng context). Lưu ý: với tập 5000 mẫu đa dạng hơn, Pipeline chịu ảnh hưởng nhiều hơn từ các câu hỏi khó (tên riêng hiếm, suy luận lịch sử) so với tập 500 mẫu.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{results/figures_5000/fig7_retriever_accuracy.png}
...
Khi so sánh \textbf{B2 (Pretrained)} và \textbf{M1 (Fine-tuned)}: EM tăng từ 44.40\% lên 56.50\% (+12.1\%), F1 tăng từ 66.60\% lên 76.10\% (+9.5\%). Sự cải thiện lớn hơn ở EM cho thấy fine-tune giúp mô hình cắt biên span chính xác hơn --- học cách loại bỏ danh xưng thừa (``Thiếu tướng'', ``Ông'') trước tên riêng --- thay vì chỉ định vị vùng thông tin (đã giỏi nhờ pretrain đa ngôn ngữ).

\subsubsection{Pipeline thấp hơn Reader đơn lẻ --- Hiện tượng Rank Penalty}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{results/figures_5000/fig6_pipeline_gain.png}
  \caption{So sánh hiệu năng Reader đơn lẻ (Oracle) và Pipeline BM25+Reader (5000 mẫu)}
  \label{fig:pipelinegain}
\end{figure}

Biểu đồ \ref{fig:pipelinegain} chỉ ra Reader đơn lẻ (M1 Oracle) đạt EM=56.5\% / F1=76.1\%, trong khi Pipeline BM25+M1 chỉ đạt EM=36.2\% / F1=48.4\% --- Pipeline thấp hơn Oracle khoảng 20 điểm EM. Với tập test 5000 mẫu đa dạng, tỷ lệ câu hỏi khó (tên riêng hiếm, câu hỏi cần suy luận) cao hơn so với tập 500 mẫu, khiến BM25 thất bại nhiều hơn trong việc tìm đúng context (Hit@3 = 93.4\%, nghĩa là $\sim$6.6\% câu hỏi Gold Context không lọt vào Top-3). Đây là minh chứng rõ ràng nhất về giới hạn của kiến trúc BM25 Retriever với tập test lớn và đa dạng --- cần thay bằng Dense Retrieval (DPR) để cải thiện.

\subsubsection{So sánh baseline và phương pháp chính}
Phương pháp chính (M1) vượt trội rõ ràng so với cả hai baseline: so với B2 (Pretrained) tăng +12.1\% EM và +9.5\% F1; so với Pipeline+B2 Reader: M1-Oracle có EM cao hơn 21.5\% và F1 cao hơn 22.9\%. Nhóm ưu tiên chọn \textbf{M1 đơn lẻ} làm phương pháp chính (đạt điểm cao nhất), và dùng \textbf{Pipeline+M1} cho kịch bản thực tế (không có Gold Context sẵn).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{results/figures_5000/fig4_error_distribution.png}
...
Trong số 5000 câu hỏi thử nghiệm, hệ thống Pipeline M1 ghi nhận \textbf{3190 mẫu dự đoán sai} (ứng với EM=36.2\%). Dựa trên chỉ số Hit@3 = 93.4\% của Retriever (tương đương 330 mẫu BM25 tìm sai đoạn văn), nhóm tiến hành phân rã rủi ro lan truyền (Root Cause Analysis):
\begin{itemize}
    \item \textbf{Lỗi do Retriever (Retriever Fault)}: Chiếm \textbf{$\sim$10.3\%} (330/3190 mẫu) --- Bộ lọc BM25 thất bại hoàn toàn trong việc nạp ngữ cảnh chứa đáp án vào Top-3, khiến Reader không có thông tin đầu vào chính xác để trích xuất.
    \item \textbf{Lỗi do Reader (Reader Fault)}: Chiếm \textbf{$\sim$89.7\%} (2860/3190 mẫu) --- BM25 nạp đúng ngữ cảnh nhưng mô hình Reader trích xuất sai biên thực thể hoặc nhầm vị trí.
\end{itemize}
\textbf{Lưu ý:} Trong số lỗi Reader, $\sim$11.7\% thực chất là nhãn nhiễu (Label noise --- lỗi do dữ liệu gốc bị paraphrase khi dịch từ SQuAD). Mức trần lý thuyết của hệ thống vì vậy chỉ đạt $\sim$88\%. Việc M1 đạt F1=76.10\% trên Oracle cho thấy hệ thống đang hoạt động khá sát với hiệu năng tối đa có thể đạt được.
