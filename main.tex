...
\begin{table}[H]
  ...
  \begin{tabular}{lcccl}
    ...
    \textbf{Pipeline: BM25 + B2} & 34.9\% & 52.4\% & $\sim$115 ms & BM25 Top-3 + Pretrained Reader. \\
    \textbf{Pipeline: BM25 + M1} & 42.4\% & 57.5\% & $\sim$115 ms & BM25 Top-3 + M1 + Rank Penalty. \\
    \bottomrule
  \end{tabular}
  }
  \label{tab:ketqua}
...
\begin{table}[H]
  ...
  \begin{tabular}{ccp{7cm}}
    ...
    K = 5 & 85.3\% & Tăng nhẹ nhưng gây rủi ro truncation cho Reader \\
    \bottomrule
  \end{tabular}
  \label{tab:recallk}
\end{table}

\textbf{Lý do chọn K=3:} Tăng K lên 5 chỉ cải thiện thêm 3.3\% nhưng làm tổng độ dài input tăng gấp 1.67 lần, tái diễn hiện tượng cắt cụt token (truncation). K=3 là điểm cân bằng tối ưu giữa coverage và chi phí tính toán.
...
Dựa trên Bảng \ref{tab:recallk}, $K=3$ là điểm cân bằng kỹ thuật lý tưởng nhất giữa độ bao phủ thông tin (Hit@3 = 82.0\%) và rủi ro truncation ngữ cảnh. Tăng K lên 5 chỉ cải thiện thêm 3.3\% (đạt 85.3\%) nhưng làm tổng input token tăng gấp 1.67 lần.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    5000 mẫu (Tập lớn) & 82.0\% & 42.4\% & 57.5\% \\
    \midrule
    \textbf{Mức độ suy giảm} & \textbf{-13.2\%} & \textbf{-11.4\%} & \textbf{-14.4\%} \\
    \bottomrule
  \end{tabular}
  \label{tab:sosanh500}
\end{table}
...
Trong số 5000 câu hỏi thử nghiệm, hệ thống Pipeline M1 ghi nhận \textbf{2878 mẫu dự đoán sai} (ứng với EM=42.4\%). Dựa trên chỉ số Hit@3 = 82.0\% của Retriever (tương đương 900 mẫu BM25 tìm sai đoạn văn), nhóm tiến hành phân rã rủi ro lan truyền (Root Cause Analysis):
\begin{itemize}
    \item \textbf{Lỗi do Retriever (Retriever Fault)}: Chiếm \textbf{$\sim$31.3\%} (900/2878 mẫu) --- Bộ lọc BM25 thất bại hoàn toàn trong việc nạp ngữ cảnh chứa đáp án vào Top-3, khiến Reader không có thông tin đầu vào chính xác để trích xuất.
    \item \textbf{Lỗi do Reader (Reader Fault)}: Chiếm \textbf{$\sim$68.7\%} (1978/2878 mẫu) --- BM25 nạp đúng ngữ cảnh nhưng mô hình Reader trích xuất sai biên thực thể hoặc nhầm vị trí.
\end{itemize}
\textbf{Lưu ý:} So với mức lỗi Retriever khá nhỏ ($\sim$10-14\%) ở tập 500 mẫu, trên tập 5000 mẫu tỷ trọng lỗi do BM25 đã tăng vọt lên tới 31.3\%. Điều này cho thấy giới hạn rõ ràng của thuật toán khớp từ khóa BM25 trước kho dữ liệu lớn, đòi hỏi hệ thống cần được nâng cấp lên Dense Retrieval trong tương lai.
