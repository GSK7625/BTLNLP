...
| :--- | :---: | :---: | :---: | :--- |
| **B2: XLM-RoBERTa Pretrained (SQuAD2)** | 44.3% | 66.5% | ~110 ms | Baseline — Zero-shot trên tiếng Việt, chưa fine-tune |
| **M1: XLM-RoBERTa Fine-tuned** | **56.5%** | **76.1%** | ~110 ms | **Phương pháp chính** — fine-tune trực tiếp trên ViSpanExtractQA sạch |
| **Pipeline: BM25 + B2 Reader** | 34.9% | 52.4% | ~115 ms | BM25 Top-3 + Pretrained Reader |
| **Pipeline: BM25 + M1 Reader** | **42.4%** | **57.5%** | ~115 ms | BM25 Top-3 + M1 + Rank Penalty |

**Nhận xét:**
- Fine-tune M1 tăng EM từ 44.3% → 56.5% (+12.2%), F1 từ 66.5% → 76.1% (+9.6%). Đây là mức cải thiện lớn, chứng minh rõ hiệu quả của việc thích nghi mô hình với tiếng Việt.
- Pipeline (BM25+M1) đạt EM=42.4% — thấp hơn M1 Oracle (56.5%) do lỗi Retriever (Hit@3 = 82.0%, 18.0% câu hỏi BM25 không tìm được đúng context). Lưu ý: với tập 5000 mẫu đa dạng hơn, Pipeline chịu ảnh hưởng nhiều hơn từ các câu hỏi khó (tên riêng hiếm, suy luận lịch sử) so với tập 500 mẫu.
- Không thể kết luận mô hình tốt chỉ dựa vào 1 con số: M1 có F1 cao (76.1%) nhưng Pipeline lại thấp hơn (57.5%), vì kiến trúc pipeline làm suy yếu điểm số do lỗi Retriever lan truyền xuống Reader.
...
*Lý do chọn K=3:* Tăng K lên 5 chỉ cải thiện thêm 3.3% nhưng làm tổng độ dài input tăng gấp 1.67 lần, tái diễn hiện tượng cắt cụt token (truncation). K=3 là điểm cân bằng tối ưu.
...
Biểu đồ 6 chỉ ra rằng Reader đơn lẻ (M1 Oracle) đạt EM=56.5% / F1=76.1%, trong khi Pipeline BM25+M1 chỉ đạt EM=42.4% / F1=57.5% — tức Pipeline thấp hơn Oracle khoảng 14 điểm EM. Đây là kết quả **hoàn toàn hợp lý về mặt lý thuyết** trong kiến trúc Open-domain QA.
...
- So với B2 (Pretrained): EM +12.2%, F1 +9.6%
- So với Pipeline+B2: EM M1-Oracle cao hơn 21.6%, F1 cao hơn 23.7%
...
Trong số 5000 câu hỏi thử nghiệm, Pipeline M1 đạt EM=42.4%, tương đương **2878 mẫu trả lời sai**. Dựa trên chỉ số Hit@3 = 82.0% của Retriever (tương đương 900 mẫu BM25 tìm sai đoạn văn), nhóm tiến hành phân rã rủi ro lan truyền (Root Cause Analysis):
