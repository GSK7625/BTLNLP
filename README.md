...
## 🛠️ 2. Kiến trúc hệ thống & Pipeline xử lý
...
graph TD
    ...
    UserQA["Người dùng (Chỉ nhập Câu hỏi)"] -->|Truy vấn| GlobalBM25["BM25 Retriever (Top-K Contexts)"]
    GlobalBM25 -->|Top-3 Đoạn văn bản| PipelineReader["Reader trên Top-3 (Chọn score cao nhất + Rank Penalty)"]
    PipelineReader -->|Kết quả tích hợp| Output
```
...
## ⚙️ 4. Quy trình tiền xử lý dữ liệu
...
## 🧠 8. Tự học & Phát triển công nghệ

Trong quá trình thực hiện đề tài, nhóm đã nghiên cứu và làm chủ các kỹ thuật:
* **Hugging Face Tokenizer (Offset Mapping)**: Áp dụng ánh xạ tọa độ ký tự sang chỉ số token để đánh nhãn vị trí bắt đầu/kết thúc cho bài toán trích xuất câu trả lời một cách chính xác.
* **Kiến trúc Retriever-Reader & Rank Penalty**: Xây dựng giải thuật tích hợp công cụ tìm kiếm BM25 với mô hình ngôn ngữ sâu, đồng thời phát triển cơ chế hình phạt điểm số dựa trên thứ hạng tìm kiếm (`Rank Penalty`) để khắc phục hiện tượng Overconfidence (quá tự tin) của Reader khi nhận ngữ cảnh sai.
* **Giao diện Web Flask**: Tự lập trình backend API và xây dựng giao diện trực quan cho phép so sánh song song kết quả và tốc độ phản hồi của các mô hình trong thời gian thực.
...
| :--- | :--- | :---: |
| **Thành viên 1** | Khảo sát bài toán, làm sạch dữ liệu, chuẩn hóa Unicode NFC và xử lý lệch Casing. | 20% |
| **Thành viên 2** | Xây dựng baseline BM25 và tích hợp bộ truy hồi câu liên quan. | 20% |
| **Thành viên 3** | Thiết kế pipeline huấn luyện XLM-RoBERTa, tinh chỉnh siêu tham số trên GPU. | 20% |
| **Thành viên 4** | Phát triển module phân tích lỗi tự động và xuất báo cáo CSV định lượng. | 20% |
| **Thành viên 5** | Xây dựng giao diện Web Flask demo, viết tài liệu README và chuẩn bị báo cáo slide. | 20% |
