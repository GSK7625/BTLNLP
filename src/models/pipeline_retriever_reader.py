...
def evaluate_pipeline(data_path: str, model_name: str, num_samples: int = None, batch_size: int = 16, top_k: int = 3):
    """
    ...
    for item in tqdm(data, desc="BM25 Retrieval"):
        ...
        # Tính Top-K accuracy (nếu đoạn văn đúng nằm trong Top-K tìm thấy)
    ...
    for idx, item in enumerate(data):
        ...
        # Chọn ứng cử viên có score (confidence) cao nhất
        best_score = -1.0
        best_pred = {'answer': '', 'score': 0.0}
        best_ctx = item_ctxs[0]
        
        for pred, ctx in zip(item_preds, item_ctxs):
...
if __name__ == '__main__':
    ...
    print("="*80)
    res_pre = evaluate_pipeline(args.data, args.model_pretrained, args.num_samples, args.batch_size, args.top_k)
    ...
    if os.path.exists(args.model_finetuned) and os.listdir(args.model_finetuned):
        ...
        print("="*80)
        res_ft = evaluate_pipeline(args.data, args.model_finetuned, args.num_samples, args.batch_size, args.top_k)
    ...
    print("="*80)
    print(f"  Kho tài liệu (Corpus)   : {res_pre['retriever_accuracy']:.2f}% Top-{args.top_k} Retrieval Accuracy")
    print("─"*80)
    ...
    print("\n  [MARKDOWN TABLE]")
    print(f"  | Mô hình Pipeline kết hợp | Retriever Top-{args.top_k} (%) | EM (%) | F1 (%) |")
    print("  |---|---|---|---|")
    print(f"  | BM25 + XLM-R Pretrained | {res_pre['retriever_accuracy']:.2f}% | {res_pre['exact_match']:.2f}% | {res_pre['token_f1']:.2f}% |")
    if results['finetuned_pipeline']:
        print(f"  | BM25 + XLM-R Fine-tuned (M1) | {res_ft['retriever_accuracy']:.2f}% | {res_ft['exact_match']:.2f}% | {res_ft['token_f1']:.2f}% |")
    print()
